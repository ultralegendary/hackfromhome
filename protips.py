prot={
    ...
    "print":'''\n\tIf you want to output a large paragraph you can use `\` in cpp to break the lines to make the paragraph visible in the screen
...
talk about their IQ being high while doing small brain things";`
    ...
    "elif":'''\t if you have only one statment after if/for/else, You can avoid the parantheses`{ }`
    `if(a<0)
    cout<<a<<"is positive";`
    instead of 
    `if(a<0)
    ...
    "for":'''You can give multiple initialization, conditions, updations inside the for loop!
    `for(int i=0,j=10;i<10,j>0;i++,j--)
    
    ''',
    "comment_line":'''As python which has multi comment line `\'\'\'` in c++we use `/* comment line */`
    ''',
